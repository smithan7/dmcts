...
public:
	...
	double get_probability() { return this->my_probability; };
	...
	void set_as_root() {this->my_probability = 1.0; };

	// call from parent not self
	void search(bool &am_root, const int &depth_in, const double &time_in, std::vector<bool> &task_status, std::vector<int> &task_set, int &rollout_depth, const int &update_index);
	void reset_task_availability_probability() { this->probability_task_available = -1.0; };
	void set_probability(const double &prob){ this->my_probability = prob; };
...
private:
	...
	void update_max_branch_reward_kid(Distributed_MCTS* gc); // check if I need to update max kid and update it and my branch reward if I have to
	void update_min_branch_reward_kid(Distributed_MCTS* gc); // check if I need to update max kid and update if I have to
	...
	double my_probability; // probability of me being selected, function of my relative reward to adjacent kids and parent's probability
