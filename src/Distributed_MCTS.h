...
public:
	...
	void set_as_root() {this->raw_probability = 1.0; this->branch_probability = 1.0; this->parent = NULL; };

	// call from parent not self
	void search(int depth_in, Distributed_MCTS* parent, std::vector<bool> &task_status, std::vector<int> &task_set, int rollout_depth, const int &update_index);
...
private:
	...
	void update_down_branch_expected_reward(const int &gc); // update my branch reward, min, max, and sum
	void update_down_branch_expected_reward(); // update my branch reward, min, max, and sum
	void update_probability_task_is_available();
	void get_difference_penalty(); // this is used for difference rewards
	...
	bool use_difference_rewards; // am I strictly reward or do I account for alternative actions?
	double travel_weight; // for costs
